    ...
    IServiceProvider _serviceProvider) : IRazorRendererHelper
{
    ...
    private IView FindView(ActionContext actionContext, string partialName)
    {
        ...
        var sharedViewPath = Path.Combine(baseDirectory, partialName.TrimStart('~', '/').Replace('/', Path.DirectorySeparatorChar));
        var getSharedResult = _viewEngine.GetView(null, sharedViewPath, false);
        if (getSharedResult.Success)
            return getSharedResult.View;

        var searchedLocations = getPartialResult.SearchedLocations
            .Concat(findPartialResult.SearchedLocations)
            .Concat(getSharedResult.SearchedLocations);
